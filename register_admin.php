	...
	if (!$username || !$password || !$email || !$first_name || !$last_name) {
		...
		try {
			if ($admin) {
				...
				// Allow first admin creation if none logged in
				require_once 'config/database.php';
				$db = DatabaseConfig::getInstance()->getConnection();
				$stmt = $db->prepare("SELECT COUNT(*) as c FROM users WHERE role='admin'");
				$stmt->execute();
				$hasAdmin = (int)$stmt->fetch()['c'] > 0;
				if ($hasAdmin) {
					$error = 'Only an existing admin can create new admins.';
				} else {
					$ins = $db->prepare("INSERT INTO users (username, password, email, first_name, last_name, role) VALUES (?, ?, ?, ?, ?, 'admin')");
					$ins->execute([$username, password_hash($password, PASSWORD_DEFAULT), $email, $first_name, $last_name]);
					$success = 'First admin created. You can now log in.';
					$_POST = [];
				}
			}
		} catch (Exception $e) {
